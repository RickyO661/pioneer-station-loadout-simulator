...
/** Per-damage-type projectile figures shown by Pioneer Station for weapons and special ammunition. */
export interface ProjectileStats { channels: { innerRaw: number; outerRaw: number; blastRadiusRaw: number }[]; }
...
export interface Item { id: number; name: string; category: ItemCategory; sourceCategory: string; description: string; weightKg: number; price: number; requirement: ItemRequirement; armor?: ArmorStats; suitModifiers?: SuitModifiers; projectile?: ProjectileStats; consumable?: ConsumableStats; rawFields: string[]; }
...
export interface Eligibility { eligible: boolean; messages: string[]; }
